...
const BusinessReviewListPage: React.FC = () => {
  ...
  const fetchPendingReviews = async () => {
    setLoading(true);
    try {
      const response = await reviewService.getPendingBusinessReviews();
  ...
  const columns: ColumnsType<PendingReview> = [
    ...
    },
    {
      ...
      width: 120,
      render: (_, record) => (
        <Button type="link" icon={<EyeOutlined />} onClick={() => navigate(`/reviews/business/${record.id}`)}>
          去审核
        </Button>
      )
    }
